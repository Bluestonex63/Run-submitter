...
button.addEventListener("click", async () => {
    ...
    for (masterdiv of document.querySelectorAll(".masterdiv")) {
        ...
        masterdiv.querySelector("#catsel").addEventListener("change", () => {lvlc(lvls); varc(vari, lvls, cats)})
    }
    button.disabled = false
})

document.querySelector("#addarun").addEventListener("click", () => {
    let run2 = document.querySelectorAll(".masterdiv")[document.querySelectorAll(".masterdiv").length-1]
    run2 = run2.cloneNode(true)
    run2.children[0].children[0].innerHTML = `Run ${Number(run2.children[0].children[0].innerHTML.split(" ")[1]) + 1}`
    console.log(document.querySelectorAll(".masterdiv")[document.querySelectorAll(".masterdiv").length-1])
